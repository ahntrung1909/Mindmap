...
const AddNodeOnEdgeDrop = ({ params }) => {
  ...
  const [mindmapName, setMindmapName] = useState("Mindmap của tôi");
  const [mindmapTitle, setMindmapTitle] = useState("Mindmap title");
  ...
  const onSave = useCallback(async () => {
    if (rfInstance) {
      ...
      const flag = await updateAction(params.id, flow);
      if (flag) {
    ...
    <div>
      <div className="1250px mx-auto px-[25px] flex justify-between">
        <div className="flex flex-col mb-[10px]">
          <input
            type="text"
            value="Mindmap không tên"
            className="text-4xl "
            onChange={handleChangeMindmapName}
          />
          <input
            type="text"
            value="Không có title"
